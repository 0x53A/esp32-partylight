use common::config::*;
use egui::{self, Button, CollapsingHeader, Color32, FontFamily, FontId, Slider};
...
) -> Result<ActorRef<HandlerMessage>, ractor_wormhole::ractor::RactorErr<()>> {
                                ...
                                match unsafe { (&*bt_ptr).read_config_raw().await } {
                                    Ok(jsv) => {
                                        ...
                                        if let Ok(cfg) = postcard::from_bytes::<AppConfig>(&vec) {
                                            ...
                                            let _ = self_actor_ref
                                                .send_message(HandlerMessage::Heartbeat);
                                        } else {
                                            let mut state = state_clone.lock().unwrap();
                                            state.last_status = "Decode error".to_string();
                                            state.conn =
                                                ConnectionStatus::Broken(AppConfig::default());
                                            state.busy = false;
                                            state.last_update = Some(Instant::now());
                                        }
                                    }
                        ...
                        spawn_local(async move {
                            ...
                            while (interval.next().await).is_some() {
                                ...
                                if let Err(_e) = hb_res {
                                    ...
                                    for _attempt in 0..3 {
                                        gloo_timers::future::sleep(Duration::from_millis(1000))
                                            .await;
...
impl PartylightApp {
        ...
        match &state.conn {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.add_enabled(!state.busy, Button::new("Write")).clicked() {
                        if let Some(cfg) = &state.config {
                            let _ = self
                                .handler
                                .send_message(HandlerMessage::Write(cfg.clone()));
                        }
                    }

                    if ui
                        .add_enabled(!state.busy, Button::new("Disconnect"))
                        .clicked()
                    {
            ...
            ConnectionStatus::Broken(_cfg) => {
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .add_enabled(!state.busy, Button::new("Reconnect"))
                        .clicked()
                    {
    ...
    fn draw_config_editor(&self, ui: &mut egui::Ui, state: &mut AppState) {
        ...
        ui.horizontal(|ui| {
            if ui.button("Stripes").clicked() {
                let _ = self
                    .handler
                    .send_message(HandlerMessage::SetConfig(AppConfig::stripes()));
                let _ = self.handler.send_message(HandlerMessage::SetStatus(
                    "Loaded Stripes preset".to_string(),
                ));
            }
            if ui.button("Bars").clicked() {
                let _ = self
                    .handler
                    .send_message(HandlerMessage::SetConfig(AppConfig::bars()));
                let _ = self
                    .handler
                    .send_message(HandlerMessage::SetStatus("Loaded Bars preset".to_string()));
            }
            if ui.button("Bars2").clicked() {
                let _ = self
                    .handler
                    .send_message(HandlerMessage::SetConfig(AppConfig::bars2()));
                let _ = self
                    .handler
                    .send_message(HandlerMessage::SetStatus("Loaded Bars2 preset".to_string()));
            }
            if ui.button("Quarters").clicked() {
                let _ = self
                    .handler
                    .send_message(HandlerMessage::SetConfig(AppConfig::quarters()));
                let _ = self.handler.send_message(HandlerMessage::SetStatus(
                    "Loaded Quarters preset".to_string(),
                ));
            }
        });
    ...
    fn draw_pattern_editor(&self, ui: &mut egui::Ui, cfg: &mut AppConfig) {
        // I want to be able to change the brightness of all channels.
        // We get the current maximum value of all color channels and all FFT channels,
        //  and let the user change them.

        let max_brightness = *cfg
            .pattern
            .get_all_channels()
            .iter()
            .map(|c: &ChannelConfig| {
        ...
        let mut current_slider_value = max_brightness;
        // show a slider ...
        ui.add(Slider::new(&mut current_slider_value, 0.1..=1.0));

        // and if the user changed it, scale all colors.
        if current_slider_value != max_brightness {
            let scale_factor = current_slider_value / max_brightness;
            for ch in cfg.pattern.get_all_channels_mut() {
                ch.color.iter_mut().for_each(|c| *c *= scale_factor);
            }
        }
    ...
    fn draw_channel_editor(
        ...
        CollapsingHeader::new(format!("{} {}", label, index))
            .default_open(true)
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.label("start:");
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add(
                        egui::widgets::DragValue::new(&mut ch.color[0])
                            .speed(0.01)
                            .range(0.0..=1.0),
                    );
                    ui.add(
                        egui::widgets::DragValue::new(&mut ch.color[1])
                            .speed(0.01)
                            .range(0.0..=1.0),
                    );
                    ui.add(
                        egui::widgets::DragValue::new(&mut ch.color[2])
                            .speed(0.01)
                            .range(0.0..=1.0),
                    );
                });
